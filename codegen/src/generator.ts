...
function collectOperations(spec: OpenAPIV3_1.Document) {
...
function buildResultSchema(operation: OperationDetails) {
  const responses = (operation as any).responses ?? operation["responses"];
...
async function writeToolsFile(
  ...
  for (const operation of operations) {
    ...
    const destructured = hasPathParams
      ? `{ ${pathNames.join(", ")}, ...args }`
      : "args";
    const argsType = `z.infer<typeof ${operation.paramConst}>`;
    ...
    parts.push(
      `export const ${operation.toolConst}: Tool = {`,
